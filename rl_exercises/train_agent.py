...
def train(cfg: DictConfig) -> float:
    ...
    if cfg.agent == "sb3":
        ...
        p_kwargs = cfg.get("policy_kwargs", {"epsilon": 0.1})
        policy = EpsilonGreedyPolicy(env=env, **p_kwargs)

        a_kwargs = cfg.get("agent_kwargs")
        agent = TDAgent(env=env, policy=policy, **a_kwargs)
