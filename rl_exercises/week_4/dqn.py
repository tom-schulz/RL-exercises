...
class DQNAgent(AbstractAgent):
    ...
    def epsilon(self) -> float:
        ...
        return final + (start - final) * np.exp(-self.total_steps / self.epsilon_decay)
    ...
    def update_agent(
        ...
        with torch.no_grad():
            qvals_future = self.target_q(s_next)
            best_future_q = torch.max(qvals_future, 1)[0]
            target = r + self.gamma * best_future_q * (1 - mask)
...
def main(cfg: DictConfig):
    ...
    agent = DQNAgent(env=env, **agent_kwargs)
